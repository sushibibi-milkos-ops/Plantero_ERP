...
describe('purchasing/invoicing', () => {
  it('mal kabulden alış faturası: 320.999 kapanır, 191 doğar, tedarikçi cari bakiyesi (320) artar', async () => {
    await withRollback(async (tx) => {
      ...
      // Mal kabul sonrası: 320.999 alacaklı 1250 (INV borçlu) — henüz fatura yok.
      expect((await probe.bal('320.999', 'VUK')).toFixed(4)).toBe('-1250.0000');

      const { invoice, lines } = await createPurchaseInvoiceFromReceipt(tx, receipt.id, ctx);
      ...
      expect(lines[0]!.vatRate).toBe('20.0000');
  ...
  it('PO bağlı mal kabul: purchase_order_lines.invoicedQty artar ve PO tamamen faturalanınca invoiced durumuna geçer', async () => {
    await withRollback(async (tx) => {
      ...
      await createPurchaseInvoiceFromReceipt(tx, receipt.id, ctx);
  ...
  it('bir mal kabul yalnızca bir kez faturalanabilir', async () => {
    await withRollback(async (tx) => {
      const b = await seedBase(tx);
      await ensurePurchaseJournal(tx);
