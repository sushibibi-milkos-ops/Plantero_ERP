...
import { listWarehouses, listLocations, listSuppliers, listProductsForPicker, getPurchaseOrderWithLines } from '@/modules/stock/queries';
...
export default async function NewReceiptPage({ searchParams }: { searchParams: Promise<{ po?: string }> }) {
  ...
  const [warehouses, locations, suppliers, products, poData] = await Promise.all([
    listWarehouses(),
    listLocations(),
    listSuppliers(),
