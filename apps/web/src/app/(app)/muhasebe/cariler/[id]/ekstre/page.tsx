...
export default async function PartnerStatementPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      ...
      <KpiStripRow>
        <KpiCard variant="strip" title="Güncel bakiye" value={runningBalance} format="money" />
        {aging ? <KpiCard variant="strip" title="0-30 gün" value={aging.buckets[0]?.amount.toFixed(4) ?? '0'} format="money" /> : null}
        {aging ? <KpiCard variant="strip" title="31-60 gün" value={aging.buckets[1]?.amount.toFixed(4) ?? '0'} format="money" /> : null}
        {aging ? <KpiCard variant="strip" title="90+ gün" value={aging.buckets[3]?.amount.toFixed(4) ?? '0'} format="money" /> : null}
      </KpiStripRow>
