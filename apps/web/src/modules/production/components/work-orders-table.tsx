...
export function WorkOrdersTable({ workOrders }: { workOrders: WorkOrderRow[] }) {
  const columns = useMemo<ColumnDef<WorkOrderRow, unknown>[]>(
    () => [
      ...
      { accessorKey: 'lineCode', header: 'Hat', meta: { width: 70, mobile: 'hidden' }, cell: ({ row }) => <span className="font-mono text-xs">{row.original.lineCode}</span> },
      ...
      { accessorKey: 'yieldPct', header: 'Verim', meta: { align: 'right', width: 80, mobile: 'hidden' }, cell: ({ row }) => (row.original.yieldPct ? <span className="num text-xs">%{Number(row.original.yieldPct).toLocaleString('tr-TR', { maximumFractionDigits: 1 })}</span> : <span className="text-muted-foreground">—</span>) },
      ...
      { accessorKey: 'plannedStart', header: 'Planlanan başlangıç', meta: { width: 130, mobile: 'hidden' }, cell: ({ row }) => <span className="text-xs text-muted-foreground">{row.original.plannedStart ? formatDate(row.original.plannedStart) : '—'}</span> },
      { accessorKey: 'operatorName', header: 'Operatör', meta: { width: 130, mobile: 'hidden' }, cell: ({ row }) => row.original.operatorName ?? <span className="text-muted-foreground">—</span> },
      { accessorKey: 'unitCost', header: 'Birim maliyet', meta: { align: 'right', width: 110, mobile: 'hidden' }, cell: ({ row }) => <MoneyCell value={row.original.unitCost} digits={4} muted={Number(row.original.unitCost) === 0} /> },
    ],
    [],
  );
