...
export function RecordPaymentForm({
  ...
  /** Girilen toplam tutarı, vadesi en yakın faturadan başlayarak sırayla dağıtır ("otomatik en eski önce"). */
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (res.ok) {
      ...
      startTransition(() => router.push('/finans/tahsilat'));
    } else {
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 pb-[calc(72px+env(safe-area-inset-bottom))] md:pb-0">
        ...
        <FormActions
          ...
          submitLabel={direction === 'inbound' ? 'Tahsilatı kaydet' : 'Ödemeyi kaydet'}
        />
      </form>
    </Form>
  );
