...
async function main() {
  ...
  try {
    for (const vp of [{ width: 1440, height: 900 }]) {
      ...
      await page.route('**/*', async (route) => {
        if (/\/kokpit(\?|$)/.test(route.request().url())) await new Promise((r) => setTimeout(r, 12000));
      ...
      while (Date.now() - t0 < 20_000) {
        ...
        if (r) {
          skel = r;
          await page.screenshot({ path: resolve(OUT, `skeleton-desktop-${vp.width}.png`), animations: 'disabled' });
          break;
        }
  ...
  writeFileSync(resolve(OUT, 'probe-r10b-desktop.json'), JSON.stringify(out, null, 2));
  console.error('yazıldı: probe-r10b-desktop.json');
}
