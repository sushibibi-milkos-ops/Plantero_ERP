...
const COLLECT = String.raw`(() => {
  ...
  var small = [];
  Array.prototype.slice.call(mainEl.querySelectorAll('button, a, input, select, textarea, [role="button"], [role="tab"], [role="checkbox"], [role="switch"]')).forEach(function (e) {
    if (!vis(e)) return;
    var r = e.getBoundingClientRect();
    if (r.width < 44 || r.height < 44) small.push({ sel: e.tagName.toLowerCase() + ' "' + String(e.textContent || e.getAttribute('aria-label') || '').trim().slice(0, 28) + '"', w: Math.round(r.width), h: Math.round(r.height) });
  });
  out.smallTargetsInMain = small.slice(0, 20);

  var fsMain = {};
  Array.prototype.slice.call(mainEl.querySelectorAll('*')).forEach(function (e) {
    if (!vis(e) || e.children.length > 0) return;
    if (!String(e.textContent || '').trim()) return;
    var k = String(Math.round(parseFloat(getComputedStyle(e).fontSize)));
    fsMain[k] = (fsMain[k] || 0) + 1;
  });
  out.fontSizesMain = fsMain;

  var shadows = [];
  all.forEach(function (e) { if (!vis(e)) return; var bs = getComputedStyle(e).boxShadow; if (bs && bs !== 'none' && !/inset/.test(bs)) shadows.push(String(e.className||'').slice(0,60)); });
  out.shadowCount = shadows.length;
  ...
  all.forEach(function (e) {
    ...
    while (p && p !== document.body) { var pcs = getComputedStyle(p); if (parseFloat(pcs.borderTopWidth) >= 0.5) { boxInBox.push(String(e.className||'').slice(0,50) + ' <= ' + String(p.className||'').slice(0,50)); break; } p = p.parentElement; }
  });
  out.borderedInsideBordered = boxInBox.length;
  out.borderedInsideBorderedSample = boxInBox.slice(0, 6);
...
function parse(argv: string[]) {
  let route = '/kalite/izlenebilirlik?lot=PL-260903-H1-01';
