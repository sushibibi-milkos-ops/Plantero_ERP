...
const ROUTES = [['sevkiyatlar','/ihracat/sevkiyatlar'],['yeni','/ihracat/sevkiyatlar/yeni']] as const;
async function main() {
  ...
  for (const [k, route] of ROUTES) {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      await page.screenshot({ path: `artifacts/critic/ihracat-r11-focus-${k}-${out.length}.png`, clip });
  ...
  await browser.close();
  writeFileSync('artifacts/critic/probe-ihracat-r11e.json', JSON.stringify(res, null, 1));
