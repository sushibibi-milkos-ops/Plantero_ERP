/** Tur 13: kapalı sevkiyatın "Fatura & kur" sekmesindeki fatura bağlantısı — 390px dokunma hedefi + dokunmatik işaret. */
...
async function main() {
  ...
  for (const [vp, w, h, mobile] of [['1440', 1440, 900, false], ['390', 390, 844, true]] as const) {
    ...
    await page.screenshot({ path: `artifacts/critic/ihracat-r13-fatura-${vp}.png`, fullPage: true, animations: 'disabled' });
    await ctx.close();
  }
  await browser.close();
  writeFileSync('artifacts/critic/probe-ihracat-r13d.json', JSON.stringify(res, null, 1));
