...
- `apps/web/src/middleware.ts`: yalnızca `plantero_session` cookie'sinin VARLIĞINA bakar (edge'de DB yok); oturumlu bir tarayıcı `/login`'e giderse **kasıtlı olarak** doğrudan `/kokpit`'e yönlendirir (form hiç render edilmez) — bu güvenlik açısından doğru bir davranıştır, DEĞİŞTİRİLMEMELİDİR. Sonucu: aynı tarayıcı bağlamında rol değiştirmek isteyen herhangi bir test/otomasyon akışı, önce gerçek bir çıkış (`destroySession` / logout) yapmadan `/login` formuna erişemez — aksi halde giriş alanları hiç görünmeyip test timeout'a düşer. Rol değiştiren her test/otomasyon ya `logout()` çağırmalı ya da yeni bir izole tarayıcı bağlamı (`browser.newContext()`/`newPage()`, çerezsiz) kullanmalı.
